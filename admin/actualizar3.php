...
<?php include ('php/cabeadmin.php');
    ...
    $usuarios= "SELECT * FROM cotizacion WHERE id_coti='$id_coti'";
?> 
...
<div class="container">
        <div class="row">
                <div class="col-lg-12">
                    ...
                    <form action="/admin/modificar3.php" method="POST" >
                        <?php $resultado = mysqli_query($conexion,$usuarios);
                                        while($row=mysqli_fetch_assoc($resultado)) {?>
                                                
                                                    
                                                    <input type="hidden"  value="<?php echo $row["id_coti"]; ?>" name="id_coti">
                                                    <label for="">Tipo de meseta  </label>
                                                    <input type="text" value="<?php echo $row["tipmes"]; ?>" name="tipmes">
                                                    <br>
                                                    <label for="">Medida X   </label>
                                                    <input type="text" value="<?php echo $row["mex"]; ?>" name="mex">
                                                    <br>
                                                    <label for="">Medida Y   </label>
                                                    <input type="text" value="<?php echo $row["mey"]; ?>" name="mey">
                                                    <br>
                                                    <label for="">Valor final  </label>
                                                    <input type="text" value="<?php echo $row["valorf"]; ?>" name="valorf">
